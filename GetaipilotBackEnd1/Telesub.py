    ...
    if sb:
        try:
            ...
            if res and res.data and res.data.get("session_file"):
                val = res.data["session_file"]
                if val.startswith("user_") or val.endswith(".session"):
                    legacy_file_path = f"sessions/{val}"
                    if os.path.exists(legacy_file_path):
                        session_target = legacy_file_path
                        is_legacy_file = True
                else:
                    session_target = StringSession(val)
        except Exception as e:
            # Do not turn a temporary database/RLS/network failure into a new,
            # unauthorized session. Caching that empty client would hide the
            # persisted Telegram session until this process restarts.
            logger.error(f"Failed to fetch session string for {user_id}: {e}")
            raise HTTPException(
                status_code=503,
                detail="Unable to restore Telegram session. Please retry.",
            ) from e
...
def sync_dashboard_telegram_id(sb: Client, user_id: str, telegram_user_id: int):
    """Keep the shared dashboard Telegram identity in both legacy stores."""
    telegram_id = int(telegram_user_id)
    sb.table("profiles").update({"telegram_user_id": telegram_id}).eq(
        "id", user_id
    ).execute()
    sb.table("app_user_subscriptions").upsert(
        {
            "user_id": user_id,
...
async def get_status(ctx: dict = Depends(get_auth_context)):
    ...
    if is_connected:
        try:
            ...
            # Authorization is the connection source of truth. A transient
            # profile lookup failure must not make every dashboard look logged out.
            logger.warning(f"Failed to get_me for {user_id}: {e}")
...
async def login_otp(request: OtpRequest, ctx: dict = Depends(get_auth_context)):
    ...
    try:
        ...
        sync_dashboard_telegram_id(sb, user_id, me.id)
